...
func main() {
	...
	addr, err := raft.NewTCPTransport("127.0.0.1:9002", nil, 2, 10*time.Second, os.Stderr)
	if err != nil {
		log.Fatalf("Failed to create transport: %v", err)
	}

	dbStore, err := raftboltdb.NewBoltStore("raft-log2.bolt")
	if err != nil {
		log.Fatalf("Failed to create log store: %v", err)
	}

	stableStore, err := raftboltdb.NewBoltStore("raft-stable2.bolt")
	if err != nil {
	...
	logrus.Info("Raft Node 2 Initialized")
...
func main() {
	...
	addr, err := raft.NewTCPTransport("127.0.0.1:9002", nil, 3, 10*time.Second, os.Stderr)
	if err != nil {
		log.Fatalf("Transport error: %v", err)
	}

	logStore, err := raftboltdb.NewBoltStore("raft-log2.bolt")
	if err != nil {
	...
	fsm := NewFSM()
	_, err = raft.NewRaft(config, fsm, logStore, stableStore, snapshotStore, addr)
	if err != nil {
		log.Fatalf("Raft init error: %v", err)
	}

	log.Println("✅ Node2 Raft ready on 127.0.0.1:9002")

	select {} // Keep node2 running
}
...
func main() {
	...
	// Snapshot store (FIXED: handle both return values)
